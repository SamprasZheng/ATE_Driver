...
class FS100AnsPacket(FS100PacketHeader):
    ...
    #     print(f"added_status_size = {self.added_status_size}")
    #     print(f"added_status = {self.added_status}")
...
class FS100:
    ...
    def traveller_pulse(self, bag, stops, cb_status):
        status = FS100.TRAVEL_STATUS_ERROR
        fs100_status = {}

        for idx, pos in enumerate(stops):
            new_data = bag.data[0:16] +\
                struct.pack('<iiiiiii', pos[0], pos[1], pos[2], pos[3], pos[4], pos[5], pos[6]) + bag.data[44:88]
            req = bag.clone(new_data)
            ans = self.transmit(req.to_bytes())
            self.errno = ans.added_status
            if ans.status != FS100.ERROR_SUCCESS:
    ...
    def move_pulse(self, cb_status, move_type, speed_class, speed, pos, robot_no=1,
             station_no=0, tool_no=0, wait=False):
        """
            move_type (int): Type of move path. One of following:
                FS100.MOVE_TYPE_JOINT_ABSOLUTE_POS,
                FS100.MOVE_TYPE_LINEAR_ABSOLUTE_POS,
        ...
        if cb_status is None: cb_status = self.travel_status_cb
        bag = FS100ReqPacket(FS100PacketHeader.HEADER_DIVISION_ROBOT_CONTROL, 0, 0x8b, move_type, 0x01, 0x02, data,
                             len(data))
        self.traveller_thread = threading.Thread(target=self.traveller_pulse, args=(bag, pos, cb_status))
        self.traveller_thread.start()
        if wait:
            # wait for all stops visited
            self.traveller_thread.join()
    ...
    def read_position(self, pos_info, robot_no=1, data_type=DATA_TYPE_CARTESIAN):
        """Read the robot position

        Args:
            pos_info (dict): Where the robot position data is stored
            robot_no (int, optional): Robot number. Defaults to 1.
            data_type (int, optional): Robot number. Defaults to 1.
        Returns:
            int: FS100.ERROR_SUCCESS for success, otherwise failure and errno attribute
                indicates the error code.
        """
        if data_type == FS100.DATA_TYPE_CARTESIAN:
            req = FS100ReqPacket(FS100PacketHeader.HEADER_DIVISION_ROBOT_CONTROL, 0, 0x75, 100 + robot_no, 0, 0x01,
                             bytearray(0), 0)
        elif data_type == FS100.DATA_TYPE_PULSE:
        ...
        if ans.status != FS100.ERROR_SUCCESS:
            ...
            remaining_data_size = ans.data_size - 20
            pos_info['pos'] = struct.unpack(f'<{remaining_data_size//4}i', ans.data[20:20+remaining_data_size])
        return ans.status
